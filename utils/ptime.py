...
class Ptime:
    ...
    The core is the python datetime object , different types firstly transport to datetime object.
    ...
    def __init__(self, time_data, style=None):
    ...
    @property
    def parse_time(self, ):
        logger.debug('Ptime from format: {:}'.format(self.style))
    ...
    def format(self, style):
        ...
        """
        logger.debug('Ptime to format: {:}'.format(style))
        ...
        if style.find('str') != -1:
            ...
            if style is 'FVCOM_str':
                ...
                logger.debug('recognized format: default str')
            ...
            logger.debug('recognized format: user defined : {:}'.format(style))
            time_out = np.empty_like(times, dtype='U48')
            for i, itime in enumerate(times):
                time_out[i] = datetime.datetime.strftime(itime, style)

        else:
            logger.debug('recognized format: float format')
    ...
    def adjust_timezone(self, hours):
        ...
        return Ptime(self.Time+time_delta)
    ...
    def format_fvcom(self, ):
        ...
        """
        logger.debug("output the time in 'FVCOM' format ...")
    ...
    @classmethod
    def get_coincide_time(cls, ptime_model, ptime_obs):
        """
        get the coincide time, which mean every time is coincide.
        :param ptime_model:
        :param ptime_obs:
        :return:
        """
        logger.info('Start to find the coincide time')
        ...
        for i, itime_model in enumerate(ptime_model.Time):
            for j, jtime_obs in enumerate(ptime_obs.Time):
                if itime_model == jtime_obs:
                    ...
                    logger.debug('time 1 : {:}'.format(itime_model))
                    logger.debug('time 2 : {:}'.format(jtime_obs))
    ...
    @classmethod
    def get_match_time(cls, ptime_model, ptime_obs, method='datetime'):
        """
        get the match time period, which mean the time start and stop is match, not all time is coincide
        :param ptime_model:
        :param ptime_obs:
        :param method:
        :return:
        ...
        if method == 'datetime':
            ...
            bool_obs = np.all([ptime_obs.Time >= time_start, ptime_obs.Time <= time_stop], axis=0)
...
def floatformat_time(time_data, kind):
    if kind.startswith('m'):
        ...
        times = time_data
        logger.warning('unrecognized time float format: {:}'.format(kind))
