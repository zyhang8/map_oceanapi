    ...
    def __init__(self, inputfile, FMT='norm'):
        ...
        if os.path.isfile(outnc_file+'XXXX'):
            ...
            if FMT == "norm":
                ...
                field_lists = []
                for itri in self.tri:
                    field_lists.append(itri[0:3])

                self.xc = self.N2E2D(self.x, field_lists)
                self.yc = self.N2E2D(self.y, field_lists)
                self.lonc = self.N2E2D(self.lon, field_lists)
                self.latc = self.N2E2D(self.lat, field_lists)
                self.h_center = self.N2E2D(self.h, field_lists)
